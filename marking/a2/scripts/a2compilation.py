...
def findSrcFolder(dir: Path) -> Optional[Path]:
    file = firstFileMatching(dir, target="Order*.java")
    print(file)
    if file:
        file = file.resolve()
...
def copyA2TestFiles(testCaseDir: Path, ws: StudentWorkspace):
    dir = findSrcFolder(ws.scratchDir)
    print(testCaseDir)
    extraJavaFiles = list(testCaseDir.resolve().glob("java/*.java"))
