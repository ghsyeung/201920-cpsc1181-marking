...
def diffWithDiff(i: Path, o: Path, append: Callable[[str], None]):
    run = subprocess.run(["diff", "-iwu", i, o], encoding="utf-8", stdout=subprocess.PIPE)
    append("Comparing to %s\n" % i)
    if (run.returncode == 0):
        append("PASS!\n\n")
    else:
...
def validateA3(testCasesDir: Path, ws: StudentWorkspace, override=False):
    ...
    allTruthFiles = sorted(testCasesDir.glob("*.truth"), key=lambda x: x.name.split(".")[0])
